...
public class AStarSearch {
  ...
  public AStarSearch(Map map) {
    this.map = map;
    graph = new AStarNode[map.tiles.length][] ;
    for (int row = 0; row < map.tiles.length; row++) {
      graph[row] = new AStarNode[map.tiles[row].length] ;
      for (int col = 0; col < map.tiles[row].length; col++) {
  ...
  private boolean searchIteration(int goalRow, int goalCol) {
    ...
    // look S
    if(currRow+1 < graph.length)
      process(currentNode, graph[currRow+1][currCol], currRow, currCol, goalRow, goalCol, newCost) ;
    // look E
    if(currCol+1 < graph[0].length)
  ...
  public ArrayList<AStarNode> search(int sourceRow, int sourceCol, int goalRow, int goalCol) {
    ...
    for(Enemy enemy : map.enemies) {
      if(graph[Math.round(enemy.pos.z/unit)][Math.round(enemy.pos.x/unit)] != null) {
        graph[Math.round(enemy.pos.z/unit)][Math.round(enemy.pos.x/unit)].closed = true;
        graph[Math.round(enemy.pos.z/unit)][Math.round(enemy.pos.x/unit)].cost = graph.length * graph[0].length;
      }

    }
      
    graph[sourceRow][sourceCol].closed = false;
